...
export function ContractAnalysisPage() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      if (!switched) {
        setErrorMessage('Failed to switch to Somnia network. Please add Somnia to MetaMask.');
        return;
      }
      ...
      if (!hasSST) {
        setErrorMessage('You need at least 1 SST token to use this feature.');
        return;
      }
      ...
      const res = await fetch(`/api/contract-analysis?${params.toString()}`);
