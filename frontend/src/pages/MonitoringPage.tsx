...
import { contractService, switchToSomnia } from '../utils/contracts';
...
export function MonitoringPage() {
  ...
  const startMonitoring = async () => {
    ...
    try {
      ...
      if (!switched) {
        setError('Failed to switch to Somnia network. Please add Somnia to MetaMask.');
        return;
      }

      // Require at least 1 SST
      const hasSST = await contractService.hasAtLeastOneSST();
      if (!hasSST) {
        setError('You need at least 1 SST token to use this feature.');
        return;
      }
      ...
      if (err.message && err.message.includes('Payment')) {
        setError('Payment required: Please ensure you have 1 SST and approve the transaction.');
